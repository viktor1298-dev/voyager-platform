...
| **Watch DB Writer** | `lib/watch-db-writer.ts` | Debounced periodic PostgreSQL sync from watch events. Syncs health status, node counts, pod counts (total/running), events, and K8s server version (via VersionApi) to clusters table. |
...
| **Resource Stream Route** | `routes/resource-stream.ts` | SSE `/api/resources/stream` — non-blocking connect, `types` query param for initial resource types, progressive per-type snapshots via `snapshot-ready` event (emitted after `ensureTypes` resolves and ObjectCache is populated), connection-scoped auto-cleanup on disconnect, socket-tracked connection limits |
