...
async function authenticateToken(request: FastifyRequest) {
  ...
  const [tokenRow] = await db
    ...
    .where(and(eq(userTokens.tokenHash, tokenHash), or(isNull(userTokens.expiresAt), sql`${userTokens.expiresAt} > now()`)))
    .limit(1)
...
export async function registerMcpRoute(app: FastifyInstance) {
  ...
  app.post('/mcp', async (request: FastifyRequest<{ Body: JsonRpcRequest }>, reply: FastifyReply) => {
    const auth = await authenticateToken(request)
    if (!auth) {
      reply.code(401).send({ error: 'Unauthorized' })
      return
      ...
      switch (toolName) {
        case 'get_logs':
          data = await caller.logs.tail({
            targets: Array.isArray(args.targets) ? (args.targets as Array<{ podName: string; namespace: string; container?: string; clusterId?: string }>) : [],
            tailLines: typeof args.tailLines === 'number' ? args.tailLines : 200,
            search: typeof args.search === 'string' ? args.search : undefined,
            levels: Array.isArray(args.levels) ? (args.levels as Array<'ERROR' | 'WARN' | 'INFO' | 'DEBUG'>) : undefined,
          })
          break
        case 'get_events':
