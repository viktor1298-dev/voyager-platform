...
import { AlertTriangle, Bell, CheckCircle, ChevronDown, ChevronRight, History, Info, Plus, Trash2, XCircle } from 'lucide-react'
...
export default function AlertsPage() {
  ...
  const alertColumns = useMemo<ColumnDef<AlertRow, unknown>[]>(() => [
    { accessorKey: 'name', header: 'Name', cell: ({ row }) => {
      ...
      const isAutoGenerated = /^[0-9a-f]{8}-[0-9a-f]{4}-/i.test(name) || /^[0-9a-f]{24,}$/i.test(name)
      if (isAutoGenerated) {
        return (
          <span className="text-[var(--color-text-primary)] font-mono text-xs cursor-help" title={name}>
            {name.slice(0, 8)}…
          </span>
        )
      }
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={cn('inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs uppercase tracking-wide font-semibold', severityBadge[sev])}>
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="text-xs text-[var(--color-text-secondary)]">
            ...
            <p className="text-xs text-[var(--color-text-dim)]">{new Date(row.original.detectedAt).toLocaleString()}</p>
          </div>
        ),
      },
      {
        accessorKey: 'status', header: 'Status',
        cell: ({ row }) => <span className={cn('inline-flex rounded-full border px-2 py-0.5 text-xs uppercase tracking-wide font-semibold', statusBadge[row.original.status])}>{row.original.status}</span>,
      },
    ],
    [expandedId],
  )
            ...
            {table.getHeaderGroups().map((group) => (
              <tr key={group.id} className="border-b border-[var(--color-border)] bg-white/[0.02]">
                {group.headers.map((header) => (
                  <th key={header.id} className="px-3 py-2 text-left text-xs uppercase tracking-wider font-mono text-[var(--color-text-dim)]">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {Object.entries(row.original.metadata).map(([key, val]) => (
                                  <div key={key} className="rounded-md border border-[var(--color-border)] bg-[var(--color-bg-card)] p-2">
                                    <p className="text-xs uppercase tracking-wide text-[var(--color-text-dim)]">{key}</p>
