...
export default function AlertsPage() {
  ...
  const alertColumns = useMemo<ColumnDef<AlertRow, unknown>[]>(() => [
    { accessorKey: 'name', header: 'Name', cell: ({ row }) => {
      ...
      if (isAutoGenerated) {
        return (
          <span className="text-[var(--color-text-primary)] font-mono text-[12px] cursor-help" title={name}>
            {name.slice(0, 8)}…
          </span>
        )
      }
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span className={cn('inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-wide font-semibold', severityBadge[sev])}>
