...
function formatTargeting(targeting: Record<string, unknown>) {
...
export default function FeatureFlagsPage() {
  ...
  const handleToggle = useCallback(async (flagId: string, nextEnabled: boolean) => {
    ...
    try {
      ...
      setFlags((prev) =>
        prev.map((flag) =>
          flag.id === flagId ? { ...flag, updatedAt: new Date().toISOString() } : flag,
        ),
      )
        ...
        cell: ({ row }) => (
          <FeatureFlagToggle
            ...
            onToggle={async (nextEnabled) => {
              await handleToggle(row.original.id, nextEnabled)
            }}
          />
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-[11px] font-mono text-[var(--color-text-muted)]">
            {formatTargeting(row.original.targeting)}
          </span>
        ),
      },
      {
            ...
            <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                ...
                <span className="font-mono text-[var(--color-text-primary)]">
                  {formatTargeting(flag.targeting)}
                </span>
