...
const clusterColumns: ColumnDef<ClusterRow, unknown>[] = [
  ...
  },
  {
    ...
    header: 'Endpoint',
    cell: ({ getValue }) => (
      <span className="text-[var(--color-text-muted)] font-mono text-xs">{getValue<string>()}</span>
    ),
  },
  {
    accessorKey: 'status',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <SectionCard icon={<Info className="h-4 w-4" />} title="AI Bring Your Own Key (BYOK)">
          ...
          >
            ...
            <div data-testid="byok-actions" className="grid grid-cols-1 gap-2 pt-2 sm:grid-cols-2">
              <button
                type="submit"
        ...
        <SectionCard icon={<Info className="h-4 w-4" />} title="About">
          <p className="text-[13px] text-[var(--color-text-secondary)] leading-relaxed mb-4">
            <strong className="text-[var(--color-text-primary)]">Voyager Platform</strong> — Unified
...
function ClusterTable({
  ...
  if (rows.length === 0) {
    return <p className="text-xs text-[var(--color-text-muted)] py-4">No clusters registered.</p>
  }
