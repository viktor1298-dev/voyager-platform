...
export function ClusterHealthIndicator({
  ...
  const dotScaleAnim =
    entry.status === 'degraded' ? [1, 1.3, 1] : entry.status === 'critical' ? [1, 1.5, 1] : 1
          ...
          <TooltipTrigger asChild>
            ...
            <motion.span
              className={cn(
                ...
                entry.status === 'critical' && 'animate-glow-critical',
                entry.status === 'degraded' && 'animate-glow-warning',
              )}
          ...
          <TooltipContent side="top" className="text-left">
            <div className="space-y-0.5 text-xs">
              ...
              <div className="text-[var(--color-text-muted)]">
                Last check: {entry.checkedAt ? timeAgo(entry.checkedAt) : '—'}
              </div>
              {entry.responseTimeMs != null && (
                <div className="text-[var(--color-text-muted)]">
