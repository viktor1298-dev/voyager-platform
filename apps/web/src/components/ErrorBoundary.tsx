...
function parseErrorMessage(raw: string): { title: string; description: string } {
    ...
    if (arr && arr.length > 0) {
      ...
      if (hasConfigField) {
        return {
          ...
          description: 'This feature requires valid cloud credentials. Please configure your connection settings first.',
        }
      }
  ...
  if (raw.includes('FORBIDDEN') || raw.includes('403')) {
    return { title: 'Access Denied', description: 'You do not have permission to view this resource.' }
  }
...
export function QueryError({ message, onRetry, title }: { message: string; onRetry?: () => void; title?: string }) {
