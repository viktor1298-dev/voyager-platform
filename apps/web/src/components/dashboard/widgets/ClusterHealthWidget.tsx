...
function ResourceBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-xs font-mono tabular-nums text-[var(--color-text-dim)] min-w-[28px] text-right">
        {pct}%
      </span>
    </div>
  )
    ...
    <div className="h-full p-4 overflow-auto space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <Link href="/clusters" className="text-xs text-[var(--color-accent)] hover:underline">
            View all →
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {clusterList.map((c) => {
            return (
              <Link
                key={c.id}
                href={`/clusters/${c.id}`}
                className="group flex items-center gap-2 px-2.5 py-2 rounded-xl border border-[var(--color-border)]/50 hover:bg-white/[0.04] transition-all"
              >
                {c.source === 'db' ? (
          ...
          {clusterList.length === 0 && (
            <span className="text-xs text-[var(--color-text-dim)] col-span-full">No clusters</span>
          )}
        </div>
      </div>
              ...
              <div className="relative h-20 w-20">
                ...
                >
                  ...
                  <circle
                    ...
                    strokeWidth={GAUGE_STROKE_WIDTH}
                    strokeLinecap="round"
                    strokeDasharray={`${(pct / 100) * GAUGE_CIRCUMFERENCE} ${GAUGE_CIRCUMFERENCE}`}
                    className="transition-all duration-700"
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-base font-bold font-mono tabular-nums text-[var(--color-text-primary)]">
                    {pct}%
                  </span>
                </div>
              </div>
        ...
        {clusterList.length > 0 && (
          <div className="mt-3 border-t border-[var(--color-border)]/40 pt-3 space-y-2">
            {clusterList.map((c, idx) => {
              ...
              const cpu =
                c.source === 'live' ? cpuPct : Math.max(5, Math.min(95, cpuPct + (seed % 30) - 15))
              const mem =
                c.source === 'live'
                  ? memPct
                  : Math.max(5, Math.min(95, memPct + ((seed * 3) % 30) - 15))
              const pcCpu =
                cpu > 80
                  ? 'var(--color-status-error)'
                  : cpu > 60
