  ...
  return (
    <div className="h-full p-4 flex flex-col">
      <div className="flex items-center justify-between mb-3">
        ...
        <Link href="/deployments" className="text-xs text-[var(--color-accent)] hover:underline">
          View all →
        </Link>
      </div>
      <div className="flex-1 overflow-auto space-y-1.5">
        ...
        {deploymentsQuery.isLoading && activeClusterId && (
        ...
        {deploymentsQuery.isError && activeClusterId && (
        ...
        {deployments.slice(0, 8).map((d, idx) => {
          ...
          return (
            <div
              key={`${d.name}-${idx}`}
              className="flex items-center gap-2 px-2.5 py-2 rounded-lg border border-[var(--color-border)]/40"
            >
              <span
                className={`h-2 w-2 rounded-full shrink-0 ${isReady ? 'bg-[var(--color-status-active)]' : 'bg-[var(--color-status-warning)]'}`}
              />
